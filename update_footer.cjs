...
`;
code = code.replace(/<ul className="space-y-5 text-sm font-bold text-brand-charcoal tracking-tight">\\s*<li><span.*?Partner with us.*?[\\s\\S]*?<\/ul>/, newFooterNav);
...
`;
code = code.replace(/<ul className="space-y-5 text-sm font-bold text-brand-charcoal tracking-tight">\\s*<li><span.*?Impact roadmap.*?[\\s\\S]*?<\/ul>/, newFooterNav2);

// Fix Staff Login button
code = code.replace(/\(window as any\)\.toggleAdmin\?\(\)/g, "(window as any).navigate('admin')");
